...
class SignUpController: UIViewController {
        ...
        Auth.auth().createUser(withEmail: email, password: password) { (user, error) in
            ...
            storageRef.putData(uploadData, metadata: nil, completion: { (_, err) in
                ...
                storageRef.downloadURL(completion: { (downloadUrl, err) in
                   ...
                   self.updateUsersDatabase(values: values)
                })
            })
        }
    }
    ...
    @objc func handleTextInputEditing() {
        let isFormValid = ValidationsHelper.isValidEmail(emailTextfield.text ?? "")
            && ValidationsHelper.isValidPassword(passwordTextfield.text ?? "")
            && ValidationsHelper.isValidUsername(usernameTextfield.text ?? "")
