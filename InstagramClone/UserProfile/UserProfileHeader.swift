...
class UserProfileHeader: BaseReusableView {
    ...
    fileprivate func checkIfUserIsCurrent() {
        guard let currentUserUid = Auth.auth().currentUser?.uid else { return }
        guard let userId = self.user?.uid else { return }
    ...
    fileprivate func setupButtonToFollow() {
    ...
    fileprivate func setupButtonToUnfollow() {
    ...
    fileprivate func setupButtonToEditProfile() {
